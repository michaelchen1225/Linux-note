...
* 檔名的搜尋
  * [尋找執行檔案的路徑 --- which](#尋找執行檔案的路徑-----which)
...
alias which='alias | /usr/bin/which --tty-only --read-alias --show-dot --show-tilde'
        ...
        /usr/bin/which
```
* 竟然會有兩個 which ，其中一個是指令別名(alias)，而另一個則是指令本身。




**範例**：請找出 history 這個指令的完整檔名
...
* history 為 bash 的內建指令，而 which 只會搜尋在 PATH 中的指令，所以會找不到。
